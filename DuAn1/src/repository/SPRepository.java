...
public class SPRepository {
    ...
    public ArrayList<SanPhamViewModel> getAll() {
        try {
            ...
            String sql = "select MaSP, TenSP, TenLH, TenRAM, TenPin, TenDL, TenMH, SoLuongTon, GiaBan, NamBH, SANPHAM.TrangThai from SANPHAM join PhanLoaiHang on SANPHAM.Id_PhanLoai = PhanLoaiHang.Id join RAM on SANPHAM.Id_Ram = RAM.Id join Pin on SANPHAM.Id_Pin = Pin.Id join DungLuong on SANPHAM.Id_DLuong = DungLuong.Id join ManHinh on SANPHAM.Id_ManHinh = ManHinh.Id";
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            
            while (rs.next()) {
                listSp.add(new SanPhamViewModel(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getInt(8), rs.getBigDecimal(9), rs.getInt(10), rs.getInt(11)));
            }
